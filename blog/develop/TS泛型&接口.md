...
从这两个例子看，它们书写的共同点都在小括号()参数之前获取传递机那里的泛型参数。使用和普通函数一致

## 类型约束

`TS`中的非常重要的特性，它允许你指定一个泛型参数必须满足的约束条件。

通过类型约束，可以限制泛型类型的能力。确保它具有我们需要的特定属性和方法
...
`Type` 是什么？Type是一个接口，并且使用到了接口中的 泛型和继承，接口详情下面有扩展
...
当你需要定义对象的形状或者创建一个可扩展的类型系统时，使用接口。
...
`implements` 关键字确保一个类满足一个或多个特定的接口契约。这意味着该类必须实现接口中定义的所有属性和方法。在使用 `implements` 时，TypeScript 会进行类型检查，确保类正确实现了其声明要实现的接口。
